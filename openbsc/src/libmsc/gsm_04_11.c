...
{
	LOGP(DLSMS, LOGL_INFO, "Received SMS RP-SUBMIT from %s to %s.\n",
		gsms->src.addr, gsms->dst.addr);
...
int gsm411_send_sms(struct gsm_subscriber_connection *conn, struct gsm_sms *sms)
{
	...
	if (rc < 0) {
		LOGP(DLSMS, LOGL_ERROR, "Error encoding SMS from %s to %s.\n",
			sms->src.addr, sms->dst.addr);
		send_signal(S_SMS_UNKNOWN_ERROR, trans, sms, 0);
		sms_free(sms);
		trans->sms.sms = NULL;
		trans_free(trans);
	...
	*rp_ud_len = rc;
	...
	LOGP(DLSMS, LOGL_INFO, "Sending SMS DELIVER from %s to %s.\n",
			sms->src.addr, sms->dst.addr);
